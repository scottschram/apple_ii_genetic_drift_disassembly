# Genetic Drift — Apple II Game Disassembly & Source Recovery

## Brad Hawthorne's Rosetta Contribution

In February 2026, [Brad Hawthorne](https://github.com/BradHawthorne) contributed an extraordinary body of work to this project. Using his Rosetta v2 toolchain and working with Claude Code, he produced a fully annotated reconstruction of the Genetic Drift source code — transforming my raw disassembly into something that reads like the original hand-written assembly I lost decades ago.

His contribution includes:
- **[`genetic_drift_source.s`](genetic_drift_source.s)** — The recovered source code: 4,500+ lines of clean 6502 assembly with 59 named zero-page variables, 67 annotated functions, and 28 documented data tables
- **[`README_BradHawthorne.md`](README_BradHawthorne.md)** — Full documentation of his toolchain, methodology, and the game's architecture
- **7 reverse engineering tools** in [`tools/`](tools/) — an enhanced disassembler, sprite extractor, memory visualizer, BASIC detokenizer, ProDOS extractor, sound extractor, and the annotation pipeline
- **32 extracted sprite images** in [`graphics/`](graphics/)
- **Game mechanics analysis** in [`analysis/game_mechanics.md`](analysis/game_mechanics.md)

My original disassembly and tools are preserved in [`schram_v1/`](schram_v1/).
...
These were the wish list from my original work — Brad Hawthorne's contribution delivered all of them. See [`tools/`](tools/) and [`README_BradHawthorne.md`](README_BradHawthorne.md).

- ~~ProDOS disk image support~~ → `tools/extract_prodos.py`
- ~~Applesoft BASIC detokenizer~~ → `tools/detokenize_basic.py`
- ~~Execution flow tracing disassembler~~ → `tools/disasm6502.py` (CFG tracing)
- ~~Automated sprite extraction and PNG rendering~~ → `tools/extract_sprites.py`
- ~~Memory access pattern visualization~~ → `tools/memviz.py`
